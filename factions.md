...
This document describes the playable factions, their technological themes, and their units.
...
* **Infantry:**
    ...
    * `Field Medic`: (Healer) Heals nearby infantry units.
    * `Agent "Whisper"`: **(Special Unit / Hero)** Cloaks when standing still. His laser rifle can neutralize (snipe) vehicle crews.
* **Vehicles:**
...
* **Aircraft:**
    ...
    * `Eagle Fighter`: Air superiority fighter.
    * `Hypersonic Bomber "Excalibur"`: Supersonic bomber.
...
* **Infantry:**
    ...
    * `Field Surgeon`: (Healer) Emits healing sonic pulses.
    * `Agent "Blitz"`: **(Special Unit / Hero)** Cloaked. Can plant C4 on buildings and temporarily disable vehicles/buildings with an EMP shot.
* **Vehicles:**
...
* **Aircraft:**
    ...
    * `Eurofighter`: Multi-role fighter (good against air and ground).
    * `Tornado`: Bomber (cluster bombs).
...
* **Aircraft:**
    ...
    * `MiG Fighter`: Fighter jet.
    * `Nuclear Bomber`: Bomber (tactical nuke).

## 4. GBA (Global Liberation Army)
...
* **Aircraft:**
    ...
    * `Junk Plane`: (Heli role) Slow, cheap, fires MGs.
    * `Plague Bringer`: (Bomber) Crop duster that drops toxin bombs.
...
* **Aircraft:**
    ...
    * `Angelic Fighter`: (Fighter) A jet that fires beams of light.
    * `"Exodus" Bomber`: (Bomber) Drops "Plagues" (damage over time on the ground).